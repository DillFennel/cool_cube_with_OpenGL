...
uniform int faceId;   // Идентификатор грани (0-5)
...
void main()
{
    if (faceType == 1) {
        ...
        // Обычные грани - радужные цвета с анимацией
        
        // Базовый цвет для каждой грани (радуга)
        float hue = float(faceId) / 6.0;
        
        // Добавляем анимацию - движение цвета по радуге
        hue = fract(hue + time * 0.2);
        
        // Создаем небольшой градиент внутри каждой грани
        vec2 center = vec2(0.5, 0.5);
        float dist = distance(TexCoord, center);
        float gradient = 0.8 + 0.2 * sin(dist * 10.0 - time * 3.0);
        
        // Преобразуем HSV в RGB
        vec3 hsvColor = vec3(hue, 0.9, gradient);
        vec3 rgbColor = hsv2rgb(hsvColor);
        
        // Добавляем мерцающий эффект
        float sparkle = 0.0;
        float sparkleFreq = 20.0;
        sparkle += sin((TexCoord.x + time) * sparkleFreq) * 0.1;
        sparkle += sin((TexCoord.y + time * 0.7) * sparkleFreq) * 0.1;
        
        rgbColor += vec3(sparkle);
...
// Создание текстуры с веселым котом (остается без изменений)
...
int main() {
    ...
    // Создание окна
    GLFWwindow* window = glfwCreateWindow(800, 600, "3D Engine - Rainbow Cat Cube", NULL, NULL);
    if (!window) {
    ...
    int faceIdLocation = glGetUniformLocation(shaderProgram, "faceId");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Рисуем каждую грань отдельно, чтобы задать правильный faceType и faceId
        for (int i = 0; i < 6; i++) {
            if (i == 1) { // Передняя грань - с котом
                glUniform1i(faceTypeLocation, 1);
            } else { // Остальные грани - радужные
                glUniform1i(faceTypeLocation, 0);
            }
            // Передаем идентификатор грани для радужного эффекта
            glUniform1i(faceIdLocation, i);
